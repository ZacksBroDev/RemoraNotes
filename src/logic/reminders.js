...
export const scheduleBirthdayNotifications = async (person, month, day, year = null) => {
  try {
    ...
    console.log(`Scheduled ${scheduledNotifications.length} birthday notifications for ${person.name}`);
...
export const cancelBirthdayNotifications = async (personId) => {
  try {
    ...
    console.log(`Cancelled ${notificationsToCancel.length} birthday notifications for person ${personId}`);
  } catch (error) {
    ...
    if (new Date(dueDate) > new Date()) {
      ...
      const identifier = await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: person.company ? `Time to check in with ${person.name} at ${person.company}` : `Time to check in with ${person.name}`,
          data: {
            type: 'followup',
            personId: person.id
          },
      ...
      console.log(`Scheduled follow-up notification for ${person.name} at ${notificationDate}`);
      return identifier;
    }
  } catch (error) {
...
export const cancelFollowupNotification = async (personId) => {
  try {
    ...
    console.log(`Cancelled ${notificationsToCancel.length} follow-up notifications for person ${personId}`);
  } catch (error) {
