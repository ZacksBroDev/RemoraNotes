...
export const logInteractionAndUpdateFollowup = async (
  personId,
  channel = null,
  summary = null
) => {
  try {
    ...
    if (followupRule && followupRule.enabled) {
      ...
      console.log(
        `Updated follow-up for person ${personId}, next due: ${followupRule.next_due}`
      );
    }
...
export const setupClientFollowup = async (
  personId,
  cadenceDays = 30,
  enabled = true
) => {
  try {
    const ruleId = await createOrUpdateFollowupRule(
      personId,
      cadenceDays,
      enabled
    );
    ...
    if (enabled) {
      ...
      if (followupRule) {
        ...
        console.log(
          `Set up follow-up cadence for person ${personId}: every ${cadenceDays} days`
        );
      }
    }
...
export const getSuggestedInteractionSummary = (person, channel) => {
  ...
  if (person.type === "client") {
    if (person.company && person.role) {
      suggestions.push(
        `Checked in with ${person.name} (${person.role} at ${person.company})`
      );
    } else {
